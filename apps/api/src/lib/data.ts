...
import { demoStore, isDemoMode, isFirebaseMode } from "./db.js";
...
export async function findUserById(id: string): Promise<User | null> {
  ...
  if (isFirebaseMode()) return firestoreStore.getUser(id);
...
export async function findUserByEmail(email: string): Promise<User | null> {
  ...
  if (isFirebaseMode()) return firestoreStore.getUserByEmail(email);
...
export async function createUser(data: {
  ...
  if (isFirebaseMode()) return firestoreStore.createUser(data);
...
export async function updateUser(
  ...
  if (isFirebaseMode()) return firestoreStore.updateUser(id, data);
  const row = await pg.queryOne(
...
export async function findEventBySlug(slug: string) {
  if (isDemoMode()) return demoStore.getEventBySlug(slug);
  if (isFirebaseMode()) return firestoreStore.getEventBySlug(slug);
...
export async function joinEvent(eventId: string, userId: string) {
  ...
  if (isFirebaseMode()) return firestoreStore.joinEvent(eventId, userId);
  return pg.queryOne(
...
export async function checkinEvent(eventId: string, userId: string) {
  ...
  if (isFirebaseMode()) return firestoreStore.checkin(eventId, userId);
  return pg.queryOne(
...
export async function setVisibility(eventId: string, userId: string, visible: boolean) {
  ...
  if (isFirebaseMode()) return firestoreStore.setVisibility(eventId, userId, visible);
  return pg.queryOne(
...
export async function getVisibleParticipants(eventId: string) {
  ...
  if (isFirebaseMode()) {
    const participants = await firestoreStore.getEventParticipants(eventId, true);
    return participants.map(({ user, card }) => ({
      id: user?.id,
      display_name: user?.display_name,
      avatar_url: user?.avatar_url,
      headline: card?.headline,
...
export async function getCard(userId: string, eventId?: string): Promise<CardProfile | null> {
  ...
  if (isFirebaseMode()) return firestoreStore.getCard(userId, eventId);
  const row = eventId
    ? await pg.queryOne(
...
export async function upsertCard(
  ...
  if (isFirebaseMode()) return firestoreStore.upsertCard(userId, eventId, data);
  const row = await pg.queryOne(
...
export async function getRecommendations(userId: string, eventId: string) {
  if (isDemoMode() || isFirebaseMode()) {
    const store = isFirebaseMode() ? firestoreStore : null;
    let recs = isDemoMode()
      ? demoStore.getRecommendations(userId, eventId)
      : await store!.getRecommendations(userId, eventId);

    if (recs.length === 0) {
      recs = await generateAgentRecommendations(userId, eventId);
    }

    return Promise.all(
      recs.map(async (r) => {
        const candidate = isDemoMode()
          ? demoStore.getUser(r.candidate_id)
          : await firestoreStore.getUser(r.candidate_id);
        const card = isDemoMode()
          ? demoStore.getCard(r.candidate_id, eventId)
          : await firestoreStore.getCard(r.candidate_id, eventId);
        return {
          ...r,
          candidate: {
            id: r.candidate_id,
...
export async function generateAgentRecommendations(userId: string, eventId: string): Promise<Recommendation[]> {
  const userCard = isFirebaseMode()
    ? await firestoreStore.getCard(userId, eventId)
    : demoStore.getCard(userId, eventId);
  if (!userCard) return [];

  const user = isFirebaseMode() ? await firestoreStore.getUser(userId) : demoStore.getUser(userId);
  const locale = user?.locale ?? "fr";
  const { agentService } = getAiRuntime(locale);

  const participants = isFirebaseMode()
    ? await firestoreStore.getEventParticipants(eventId, true)
    : demoStore.getEventParticipants(eventId, true);

  const candidates = participants.filter((p) => p.user?.id !== userId && p.card);
  ...
  if (isFirebaseMode()) {
    await firestoreStore.setRecommendations(top);
  } else {
    demoStore.setRecommendations(top);
  }

  return top;
}

/** @deprecated Use generateAgentRecommendations */
export async function generateDemoRecommendationsWithAgents(userId: string, eventId: string) {
...
export async function updateRecommendationStatus(id: string, userId: string, status: string) {
  ...
  if (isFirebaseMode()) return firestoreStore.updateRecommendation(id, userId, status);
...
export async function createConnection(data: {
  ...
  if (isFirebaseMode()) {
    return firestoreStore.addConnection({
      ...data,
      status: "pending",
      event_id: data.event_id ?? undefined,
    });
...
export async function getConnections(userId: string, eventId?: string) {
  ...
  if (isFirebaseMode()) return firestoreStore.getConnections(userId, eventId);
  const result = await pg.query(
...
export async function saveAgentMemory(userId: string, eventId: string | null, message: string, metadata: unknown) {
  ...
  if (isFirebaseMode()) return firestoreStore.saveAgentMemory(userId, eventId, message, metadata);
  await pg.query(
...
export async function getAgentMemories(userId: string, eventId?: string) {
  if (isDemoMode()) return [];
  if (isFirebaseMode()) return firestoreStore.getAgentMemories(userId, eventId);
  const result = await pg.query(
...
export async function recordConsents(userId: string) {
  if (isDemoMode()) return;
  if (isFirebaseMode()) return firestoreStore.recordConsents(userId);
  await pg.query(
...
export async function recordConsent(userId: string, consentType: string, granted: boolean) {
  if (isDemoMode()) return;
  if (isFirebaseMode()) return firestoreStore.recordConsent(userId, consentType, granted);
  await pg.query(
...
export async function refreshRecommendations(eventId: string, userId: string) {
  if (isDemoMode()) {
    ...
    await generateAgentRecommendations(userId, eventId);
    return;
  }
  if (isFirebaseMode()) {
    await firestoreStore.clearRecommendations(userId, eventId);
    await generateAgentRecommendations(userId, eventId);
    return;
  }
  await runMatching(eventId, userId);
